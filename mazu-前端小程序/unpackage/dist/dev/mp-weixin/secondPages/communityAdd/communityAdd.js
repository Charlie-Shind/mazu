...
const _sfc_main = {
  ...
  methods: {
    ...
    uploadImages() {
      this.imageList.forEach((item, index) => {
        ...
        common_vendor.index.uploadFile({
          ...
          fail: (err) => {
            common_vendor.index.__f__("log", "at secondPages/communityAdd/communityAdd.vue:125", `Image upload failed`, err);
          }
        });
      });
    },
    ...
    addButton() {
      if (!this.title || !this.txt || !this.uploadImage || !this.grid_id || !store_userInfo.useUserInfoStore().$state.userInfo.id) {
        common_vendor.index.showToast({
          title: "请填写所有必填项",
          icon: "none"
        });
