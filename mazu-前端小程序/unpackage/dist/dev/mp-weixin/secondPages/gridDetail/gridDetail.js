...
const _sfc_main = {
  ...
  onLoad(option) {
    ...
    const apiMapping = {
      ...
      "妈祖文化": apis_index.getindexMazuCultureItemAPI,
      ...
      "两岸交流": apis_index.getindexTourismItemAPI
    };
    const apiFunction = apiMapping[option.pageGrid];
    common_vendor.index.__f__("log", "at secondPages/gridDetail/gridDetail.vue:68", option.pageGrid);
    if (apiFunction) {
      apiFunction({
        id: option.pageId
      }).then((res) => {
        ...
        common_vendor.index.__f__("error", "at secondPages/gridDetail/gridDetail.vue:80", "获取数据时出错:", error);
      });
    }
  },
  components: {
