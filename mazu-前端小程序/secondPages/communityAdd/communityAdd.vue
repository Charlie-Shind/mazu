<template>
	...
	<view class="content">
		<view class="richtext">
			...
			<piaoyiEditor :values="values" :maxlength="3000" :readOnly="readOnly" :photoUrl="photoUrl" :api="api"
				:name="name" @changes="saveContens" />
		</view>
		<view class="uploadImage">
			...
			<view class="" v-if="imageList.length<9" @click="chooseImage">
				<image class="chooseSvg" src="/static/add.svg" mode="">
				</image>
			</view>
		</view>
		<view class="button">
			<view class="select" @click="selectTopic">
				<picker class="picker" @change="changePickerUnit($event,unitList)" :value="indexUnit" :range="unitList"
					:range-key="'name'">
					<view class="picker">
			...
			<view class="add" @click="addButton">
				发布
			</view>
		</view>
	</view>
	<uni-popup ref="popup" type="dialog">
		<uni-popup-dialog :type="msgType" cancelText="取消" confirmText="确定" title="提示" content="确定要删除此图片吗?"
...
<script>
	...
	export default {
		...
		methods: {
			...
			addButton() {
				if (!this.title || !this.txt || !this.uploadImage || !this.grid_id || !useUserInfoStore().$state.userInfo
					.id) {
					uni.showToast({
						title: '请填写所有必填项',
						icon: "none"
